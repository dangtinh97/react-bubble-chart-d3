...
var BubbleChart = function (_Component) {
  ...
  _createClass(BubbleChart, [{
    ...
    value: function renderBubbles(width, nodes, color) {
      ...
      //set text
      node.append("text").attr("class", "label-text").style("font-size", function (d) {
      ...
      node.selectAll(".label-text").append('tspan').text(function (d) {
        return d.label.split("\n")[0];
      });

      node.selectAll(".label-text").append('tspan').attr('dy', function (d) {
        if (typeof d.data.fontSize !== "undefined") {
